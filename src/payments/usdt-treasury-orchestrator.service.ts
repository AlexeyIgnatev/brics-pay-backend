...
export class UsdtTreasuryOrchestratorService implements OnModuleInit {
  ...
  onModuleInit(): void {
    if (!this.hasRuntimeConfig()) {
      this.logger.warn('USDT treasury orchestrator is disabled because RPC/TOKEN/treasury env is missing');
      return;
    }
  ...
  private getRuntime(): UsdtRuntime {
    ...
    const treasuryAddress = this.cryptoService.trxAddressFromPrivateKey(treasuryPrivateKey);
    const TronWebCtor = TronWeb as unknown as new (options: { fullHost: string; privateKey: string }) => any;
    const tronWeb = new TronWebCtor({ fullHost: rpcUrl, privateKey: treasuryPrivateKey });
    this.runtime = { rpcUrl, tokenAddress, treasuryPrivateKey, treasuryAddress, tronWeb };
    return this.runtime;
  }
  ...
  private fallbackIdempotencyKey(scope: string, parts: unknown[]): string {
    const raw = [scope, ...parts].map((part) => {
  ...
  private sameAddress(left: string, right: string): boolean {
    try {
      return this.toTronHex(left).toLowerCase() === this.toTronHex(right).toLowerCase();
    } catch {
  ...
  private async ensureCustomer(customerId: number, address?: string, privateKey?: string) {
    const existing = await this.getCustomer(customerId);
    if (existing) return existing;
    const generated = this.cryptoService.trxAddressFromPrivateKey(privateKey ?? this.getRuntime().treasuryPrivateKey);
  ...
  private async sendUsdt(
    ...
    const txHash = await contract.transfer(toAddress, amountSun.toString()).send({
  ...
  private async waitForConfirmation(txHash: string, attempts = 3, delayMs = 2000): Promise<boolean> {
  ...
  private async findOperationByIdempotencyKey(idempotencyKey: string): Promise<PaymentOperation | null> {
    return this.prisma.paymentOperation.findUnique({ where: { idempotency_key: idempotencyKey } });
  }

  private async findOperationByTxHash(txHash: string): Promise<PaymentOperation | null> {
    return this.prisma.paymentOperation.findFirst({ where: { tx_hash: txHash } });
  }

  private async updateOperation(
    id: number,
    data: Partial<Pick<PaymentOperation, 'status' | 'tx_hash' | 'last_error' | 'payload' | 'attempt_count' | 'reversal_of_id'>>,
  ): Promise<PaymentOperation> {
    return this.prisma.paymentOperation.update({ where: { id }, data: data as any });
  }
  ...
  private async upsertOperationByKey(input: Parameters<UsdtTreasuryOrchestratorService['createOperation']>[0]): Promise<PaymentOperation> {
    const existing = await this.findOperationByIdempotencyKey(input.idempotency_key);
    if (existing) return existing;
    return this.createOperation(input);
  }

  private async markFailed(op: PaymentOperation, error: unknown): Promise<PaymentOperation> {
  ...
  private async markBroadcasted(op: PaymentOperation, txHash: string, payload?: UsdtPaymentPayload): Promise<PaymentOperation> {
    return this.updateOperation(op.id, {
      status: PaymentOperationStatus.BROADCASTED,
      tx_hash: txHash,
      payload: (payload ?? (op.payload as UsdtPaymentPayload | undefined) ?? undefined) as any,
      attempt_count: op.attempt_count + 1,
      last_error: null,
    });
  }

  private async markConfirmed(op: PaymentOperation, payload?: UsdtPaymentPayload): Promise<PaymentOperation> {
    return this.updateOperation(op.id, {
  ...
  private async maybeSweepCustomerWallet(customerId: number, idempotencyHint: string): Promise<void> {
    const customer = await this.getCustomer(customerId);
    if (!customer?.address || !customer.private_key) return;

    const liveBalance = await this.getUsdtBalance(customer.address);
    if (liveBalance <= SWEEP_THRESHOLD) {
      return;
    }

    const sweepKey = this.fallbackIdempotencyKey('usdt-sweep', [customerId, idempotencyHint, customer.address, liveBalance]);
    ...
    const op = existing ?? await this.createOperation({
      ...
      asset: 'USDT_TRC20',
      amount: liveBalance,
      payload: { reason: 'balance_threshold', threshold: SWEEP_THRESHOLD, live_balance: liveBalance },
      status: PaymentOperationStatus.NEW,
    });

    try {
      const { txHash } = await this.sendUsdt(customer.private_key, this.getRuntime().treasuryAddress, liveBalance);
      await this.markBroadcasted(op, txHash, { ...((op.payload as UsdtPaymentPayload) ?? {}), sweep: true });
      if (await this.waitForConfirmation(txHash)) {
        await this.markConfirmed(op);
      }
    } catch (error) {
      await this.markFailed(op, error);
      this.logger.warn(`USDT sweep failed for customer=${customerId}: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async processInternalTransfer(input: {
    ...
    const idempotencyKey = this.normalizeKey(input.idempotencyKey)
      ?? this.fallbackIdempotencyKey('usdt-internal', [input.senderCustomerId, input.receiverCustomerId, input.amount, input.senderAddress, input.receiverAddress]);

    const existing = await this.findOperationByIdempotencyKey(idempotencyKey);
    if (existing?.payload && typeof existing.payload === 'object' && 'transaction_id' in existing.payload && existing.status === PaymentOperationStatus.CONFIRMED) {
    ...
    try {
      const result = await this.prisma.$transaction(async (tx) => {
        const senderBalance = await tx.userAssetBalance.findUnique({
          where: { customer_id_asset: { customer_id: input.senderCustomerId, asset: 'USDT_TRC20' } },
        });
        ...
        await tx.userAssetBalance.upsert({
          where: { customer_id_asset: { customer_id: input.senderCustomerId, asset: 'USDT_TRC20' } },
          create: { customer_id: input.senderCustomerId, asset: 'USDT_TRC20', balance: (-input.amount).toString() },
          update: { balance: { decrement: input.amount.toString() } },
        });
        await tx.userAssetBalance.upsert({
          where: { customer_id_asset: { customer_id: input.receiverCustomerId, asset: 'USDT_TRC20' } },
          create: { customer_id: input.receiverCustomerId, asset: 'USDT_TRC20', balance: input.amount.toString() },
        ...
        await tx.paymentOperation.update({
          ...
          data: {
            status: PaymentOperationStatus.CONFIRMED,
            payload: { ...(input.payload ?? {}), transaction_id: transaction.id, confirmed: true },
            attempt_count: op.attempt_count + 1,
            last_error: null,
          },
        });
  ...
  async processWithdraw(input: {
    ...
    const idempotencyKey = this.normalizeKey(input.idempotencyKey)
      ?? this.fallbackIdempotencyKey('usdt-withdraw', [input.customerId, input.amount, input.address]);

    const existing = await this.findOperationByIdempotencyKey(idempotencyKey);
    if (existing?.status === PaymentOperationStatus.CONFIRMED && existing.tx_hash) {
      const tx = await this.prisma.transaction.findFirst({ where: { tx_hash: existing.tx_hash } });
    ...
    try {
      await this.prisma.$transaction(async (tx) => {
        const balance = await tx.userAssetBalance.findUnique({
          where: { customer_id_asset: { customer_id: input.customerId, asset: 'USDT_TRC20' } },
        });
      ...
      const { txHash } = await this.sendUsdt(this.getRuntime().treasuryPrivateKey, input.address, input.amount);
      await this.markBroadcasted(op, txHash, { ...(input.payload ?? {}), tx_hash: txHash });

      if (await this.waitForConfirmation(txHash)) {
        await this.markConfirmed(op, { ...(input.payload ?? {}), tx_hash: txHash, confirmed: true });
        const transaction = await this.createWithdrawTransaction(input.customerId, input.amount, input.address, txHash);
        await this.updateOperation(op.id, {
          payload: { ...(input.payload ?? {}), tx_hash: txHash, transaction_id: transaction.id },
        });
      ...
      await this.markFailed(op, error);
      await this.compensateWithdraw(op, input.customerId, input.amount, input.payload);
      throw error;
    }
  }

  private async compensateWithdraw(op: PaymentOperation, customerId: number, amount: number, payload?: UsdtPaymentPayload): Promise<void> {
    const existing = await this.findOperationByIdempotencyKey(`comp:${op.id}`);
    if (existing?.status === PaymentOperationStatus.CONFIRMED) {
      return;
    }

    const compensation = existing ?? await this.createOperation({
    ...
    await this.prisma.$transaction(async (tx) => {
      await tx.userAssetBalance.upsert({
        where: { customer_id_asset: { customer_id: customerId, asset: 'USDT_TRC20' } },
        create: { customer_id: customerId, asset: 'USDT_TRC20', balance: amount.toString() },
  ...
  async handleUsdtDepositWebhook(dto: {
    tx_hash: string;
    from_address: string;
    to_address: string;
    amount: number;
    ...
    const customer = await this.prisma.customer.findFirst({
      ...
      select: { customer_id: true, address: true, private_key: true, status: true },
    });
    if (!customer) {
    ...
    const idempotencyKey = this.normalizeKey(dto.idempotency_key)
      ?? this.fallbackIdempotencyKey('usdt-deposit', [dto.tx_hash]);

    const existing = await this.findOperationByIdempotencyKey(idempotencyKey) ?? await this.findOperationByTxHash(dto.tx_hash);
    if (existing?.status === PaymentOperationStatus.CONFIRMED) {
      return new StatusOKDto(existing.id);
    }
    if (existing?.payload && typeof existing.payload === 'object' && 'transaction_id' in existing.payload) {
      return new StatusOKDto(existing.id);
    }
    ...
    const op = existing ?? await this.createOperation({
      ...
      tx_hash: dto.tx_hash,
      payload: dto.payload ?? {},
      status: confirmed ? PaymentOperationStatus.BROADCASTED : PaymentOperationStatus.NEW,
    });
    ...
    await this.prisma.$transaction(async (tx) => {
      await tx.userAssetBalance.upsert({
        where: { customer_id_asset: { customer_id: customer.customer_id, asset: 'USDT_TRC20' } },
        create: { customer_id: customer.customer_id, asset: 'USDT_TRC20', balance: dto.amount.toString() },
      ...
      await tx.paymentOperation.update({
        ...
        data: {
          status: PaymentOperationStatus.CONFIRMED,
          payload: { ...(dto.payload ?? {}), transaction_id: transaction.id, confirmed: true },
          last_error: null,
        },
      });
    });
  ...
  async reconcileUsdtOperations(): Promise<StatusOKDto> {
    ...
    try {
      const ops = await this.prisma.paymentOperation.findMany({
        where: {
          operation_type: { in: [PaymentOperationType.DEPOSIT, PaymentOperationType.INTERNAL_TRANSFER, PaymentOperationType.WITHDRAW, PaymentOperationType.SWEEP, PaymentOperationType.COMPENSATION] },
          status: { in: [PaymentOperationStatus.NEW, PaymentOperationStatus.RESERVED, PaymentOperationStatus.DB_COMMITTED, PaymentOperationStatus.BROADCASTED, PaymentOperationStatus.FAILED] },
        },
      ...
      for (const op of ops) {
        try {
          if (op.operation_type === PaymentOperationType.DEPOSIT) {
            if (op.tx_hash && await this.isConfirmedTx(op.tx_hash)) {
              if (op.status !== PaymentOperationStatus.CONFIRMED) {
                ...
                await this.prisma.$transaction(async (tx) => {
                  await tx.userAssetBalance.upsert({
                    where: { customer_id_asset: { customer_id: op.customer_id, asset: 'USDT_TRC20' } },
                    create: { customer_id: op.customer_id, asset: 'USDT_TRC20', balance: op.amount.toString() },
          ...
          if (op.operation_type === PaymentOperationType.INTERNAL_TRANSFER) {
            if (op.status !== PaymentOperationStatus.CONFIRMED && op.payload && typeof op.payload === 'object' && 'transaction_id' in op.payload) {
              await this.updateOperation(op.id, { status: PaymentOperationStatus.CONFIRMED });
            }
            continue;
          }
          ...
          if (op.status === PaymentOperationStatus.DB_COMMITTED || op.status === PaymentOperationStatus.RESERVED || op.status === PaymentOperationStatus.NEW) {
            if (!op.tx_hash) {
              const customer = await this.getCustomer(op.customer_id);
              if (!customer) continue;
              const sourcePrivateKey = op.operation_type === PaymentOperationType.SWEEP
                ? customer.private_key
                : this.getRuntime().treasuryPrivateKey;
              const destinationAddress = op.operation_type === PaymentOperationType.WITHDRAW
                ? op.to_address
                : this.getRuntime().treasuryAddress;
              const { txHash } = await this.sendUsdt(sourcePrivateKey, destinationAddress, Number(op.amount));
          ...
          if (op.operation_type === PaymentOperationType.WITHDRAW) {
            await this.compensateWithdraw(op, op.customer_id, Number(op.amount), (op.payload as UsdtPaymentPayload | undefined) ?? undefined);
          }
        }
      }
