...
import {
  AdminActionLogsQueryDto,
  AdminActionLogsResponseDto,
} from './dto/admin-action-log.dto';
...
export class AdminActionLogController {
  ...
  async list(
    @Query() query: AdminActionLogsQueryDto,
  ): Promise<AdminActionLogsResponseDto> {
    const where: any = {};
    if (query.admin_id != null) where.admin_id = query.admin_id;
    if (query.action_query)
      where.action = { contains: query.action_query, mode: 'insensitive' };

    const orderBy = {
      [query.sort_by || 'createdAt']: query.sort_dir || 'desc',
    } as any;
